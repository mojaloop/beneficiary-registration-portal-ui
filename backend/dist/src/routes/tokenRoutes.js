...
const KYCInformation_1 = require("../models/KYCInformation");
const KYCData_1 = require("../models/KYCData");
...
router.post('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        // Get ESigNet token
        const esignetToken = yield (0, TokenController_1.getUserToken)(req.body.code, CLIENT_ID, jwtToken, RETURN_URL);
        if (esignetToken.error) {
            return res.status(500).json({ error: 'Error getting ESigNet token' });
        }
        // Get user data
        const userToken = yield (0, TokenController_1.fetchUserData)(esignetToken);
        if (userToken.error) {
        ...
        const userData = new KYCInformation_1.KYCInformation(userTokenData);
        // Get party information from MojaLoo SDK
        const getPatiesData = yield (0, TokenController_1.GetParties)(req.body.selectedPaymentType, req.body.payeeId);
        if (getPatiesData.error) {
            return res.status(500).json({ error: 'Error fetching user data from SDK' });
        }
        // Create party data object
        const kycData = new KYCData_1.KYCData(getPatiesData.kycData);
        // Compare user and party information
        const isMatch = yield (0, TokenController_1.compareKYCData)(userData, kycData);
        if (!isMatch) {
            return res.status(400).json({ error: 'KYC information does not match' });
        }
        // Register token and account with the payment adapter
        const token = yield (0, TokenController_1.registerToken)(req.body.selectedPaymentType, req.body.payeeId, userData.sub);
        if (!token) {
            return res.status(500).json({ error: 'Error registering token' });
        }
        const account = yield (0, TokenController_1.registerAccount)(req.body.selectedPaymentType, req.body.payeeId);
        if (!account) {
        ...
        const success = yield (0, SaveService_1.SaveDataToDB)(tokenData);
        if (success) {
